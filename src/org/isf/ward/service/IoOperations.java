	...
	public boolean updateWard(Ward ward,boolean isConfirmedOverwriteRecord) throws OHException{
		...
		if(isCodePresent(ward.getCode())) {			//ok the record is present, it was not deleted
			try {
				...
				Object[] objects = {ward.getDescription(),ward.getTelephone(),ward.getFax(),ward.getEmail(),ward.getBeds(),ward.getNurs(),ward.getDocs(),ward.isPharmacy()? 1:0,ward.isMale(),ward.isFemale(),ward.getCode()};

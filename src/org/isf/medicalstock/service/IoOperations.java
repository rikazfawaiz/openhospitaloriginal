...
public class IoOperations {
	 ...
	 * @throws OHException
	 * @deprecated use {@link #prepareChargingMovement()} or {@link #prepareDischargingMovement()} 
	 */
	@Deprecated
	 ...
	 * 
	 * @deprecated use {@link #prepareChargingMovements()} or {@link #prepareDischargingMovements()} 
	 * instead.
	 */
	@Deprecated
	 ...
	 * @throws OHException if an error occurs during the store operation.
	 */
	public boolean prepareDischargingMovement(DbQueryLogger dbQuery, Movement movement) throws OHException {
	...
	protected boolean storeMovement(DbQueryLogger dbQuery, Movement movement, String lotCode) throws OHException
	{
		try {
			...
			if (movement.getType().getType().contains("+")) origin = movement.getOrigin();
	...
	public ArrayList<Lot> getLotsByMedical(Medical medical) throws OHException {
		...
		try {
			...
			String chargeQuery = "select LT_ID_A,LT_PREP_DATE,LT_DUE_DATE,SUM(MMV_QTY) as qty,LT_COST from "
					...
					+ "where (MDSR_ID=? and MMVT_TYPE LIKE '+%') group by LT_ID_A order by LT_DUE_DATE";
			...
			for (Lot lot : lots) {
				...
				String dischargeQuery = "select SUM(MMV_QTY) as qty2 from "
						...
						+ "where (MMVT_TYPE LIKE '-%') and (MMV_LT_ID_A=?) group by MMV_LT_ID_A order by MMV_QTY desc";

...
public class IoOperations {
	...
	public int getProgYear(int year) throws OHException {
		...
		if (year == 0) {
			...
			sqlString.append(" WHERE YEAR(OPD_DATE_VIS) = ?");
	 ...
	 * @throws OHException
	 */
	public Boolean isExistOpdNum(int opdNum, int year) throws OHException {
		Boolean progYear;
		DbQueryLogger dbQuery = new DbQueryLogger();
		ResultSet resultSet = null;
		StringBuilder sqlString = new StringBuilder("SELECT OPD_PROG_YEAR FROM OPD ");
		sqlString.append(" WHERE OPD_PROG_YEAR = ?");
		List<Object> parameters = new ArrayList<Object>();
		parameters.add(opdNum);
		
		if (year == 0) {
			resultSet = dbQuery.getDataWithParams(sqlString.toString(), parameters, true);
		} else {
			sqlString.append(" AND YEAR(OPD_DATE_VIS) = ?");
			parameters.add(year);
			resultSet = dbQuery.getDataWithParams(sqlString.toString(), parameters, true);
		}
